...
//
// Stan model for partially pooled linear regression including priors, and it has a quadratic element to it  
//
...
transformed data{
		vector [N] x2;
	for (i in 1:N){
	x2[i] = (x[i])*(x[i]);											// data is squared for quadtratic term 
  	}
}
...
parameters {
	...
	real beta_q;										//quadratic term so the line can bend 
...
model{
  ...
  vector[n_vars] var_alpha;					// a new alpha for each variety, which includes grand alpha and effect of variety 
	...
	beta_q ~ normal(0, 1); 					//this is the prior used in Rethinking 
	...
	for (i in 1:N){
		ymu[i] = var_alpha[variety[i]] + var_beta[variety[i]] * x[i]+ beta_q * x2[i];
