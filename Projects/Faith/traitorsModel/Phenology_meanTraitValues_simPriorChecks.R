...
#Run model
	mdl.phen <- stan('stan/joint_3cue_phenoonly.stan',
                     data = pheno_data, warmup=4000, iter = 6000, cores = 4 )
...
postMeanSLAdf <- data.frame(postMeanSLA)
  ...
  cueEffectPlot <- mcmc_intervals(cueEffects) + 
     theme_classic() + 
      ...
      hist(postMeanSLAdf$muPhenoSp, main = paste("muPhenoSp is " , muPhenoSp, sep = ""))
      abline(v = muPhenoSp, col="red", lwd=3, lty=2)

      hist(postMeanSLAdf$muForceSp, main = paste("muForceSp is " , muForceSp, sep = ""))
      abline(v = muForceSp, col="red", lwd=3, lty=2)

      hist(postMeanSLAdf$muChillSp, main = paste("muChillSp is " , muChillSp, sep = ""))
      abline(v = muChillSp, col="red", lwd=3, lty=2)

      hist(postMeanSLAdf$muPhotoSp, main = paste("muPhotoSp is " , muPhotoSp, sep = ""))
      abline(v = muPhotoSp, col="red", lwd=3, lty=2)

      hist(postMeanSLAdf$sigmapheno_y, main = paste("sigmapheno_y is " , sigmapheno_y, sep = ""))
      abline(v = sigmapheno_y, col="red", lwd=3, lty=2)

      hist(postMeanSLAdf$betaTraitxForce, main = paste("betaTraitxForce is " , betaTraitxForce, sep = ""))
      abline(v = betaTraitxForce, col="red", lwd=3, lty=2)

      hist(postMeanSLAdf$betaTraitxChill, main = paste("betaTraitxChill is " , betaTraitxChill, sep = ""))
      abline(v = betaTraitxChill, col="red", lwd=3, lty=2)

      hist(postMeanSLAdf$betaTraitxPhoto, main = paste("betaTraitxPhoto is " , betaTraitxPhoto, sep = ""))
      abline(v = betaTraitxPhoto, col="red", lwd=3, lty=2)

      hist(postMeanSLAdf$sigmaChillSp, main = paste("sigmaChillSp is " , sigmaChillSp, sep = ""))
      abline(v = sigmaChillSp, col="red", lwd=3, lty=2)

      hist(postMeanSLAdf$sigmaForceSp, main = paste("sigmaForceSp is " , sigmaForceSp, sep = ""))
      abline(v = sigmaForceSp, col="red", lwd=3, lty=2)

      hist(postMeanSLAdf$sigmaPhotoSp, main = paste("sigmaPhotoSp is " , sigmaPhotoSp, sep = ""))
