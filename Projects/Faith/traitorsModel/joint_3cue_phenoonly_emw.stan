...
//both models --------------------------------------------------------
    ...
    real prior_betaTraitxForce; // changed! Need to add priors
    real prior_betaTraitxChill; // changed! Need to add priors
    real prior_betaTraitxPhoto; // changed! Need to add priors
}
...
model{ 
    ...
    yPhenoi[i] ~ normal( alphaPhenoSp[species[i]] + 
			 (alphaForceSp[species[i]] + betaTraitxForce * (alphaTraitSp[i]))* forcei[i] + 
			 (alphaPhotoSp[species[i]] + betaTraitxPhoto* ( alphaTraitSp[i])) * photoi[i] + 
			 (alphaChillSp[species[i]] + betaTraitxChill* (alphaTraitSp[i]) * chilli[i], 
			  sigmapheno_y);
        }
