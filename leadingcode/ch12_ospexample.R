...
m_partpool <- map2stan(
  alist(
    resp ~ dnorm(mu, sigma),
    mu <- a + a[species] + bf*force,
    a[species] ~ dnorm(0, sigma_species),
    a ~ dnorm(0, 50),
    bf ~ dnorm(0,50),
    sigma ~ dnorm(0,10),
...
m_partpool_wstudy <- map2stan(
  alist(
    resp ~ dnorm(mu, sigma),
    mu <- a + a[species] + a[study] + bf*force,
    a[species] ~ dnorm(0, sigma_species),
    a[study] ~ dnorm(0, sigma_study),
    a ~ dnorm(0, 50),
    bf ~ dnorm(0, 50),
    sigma ~ dnorm(0,10),
    sigma_species ~ dnorm(0,10),
...
if(FALSE){
  m_partpool <- map2stan(
    alist(
      resp ~ dnorm(mu, sigma),
      mu <- a[species] + bf*force + bp*photo + bc*chill,
      a[species] ~ dnorm(a, sigma),
      a ~ dnorm(0, 50),
      bf ~ dnorm(0,50),
      bp ~ dnorm(0,50),
      bc ~ dnorm(0,50),
